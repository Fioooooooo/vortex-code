...
主进程 SHALL 通过 `settings:agents:detectStatus` 检测系统中每个 registry agent 的安装状态，返回 `AgentStatus[]`，每项包含 `id`、`installed`、`detectedVersion`（可选）、`managedBy`（`"fyllocode" | "user" | null`）。
...
主进程 SHALL 通过 `npm install -g <package>` 安装 `distribution.npx` 类型的 agent，安装前检测 npm 环境，安装过程中通过 `settings:agents:installProgress` 推送进度，完成后写入 `installed.json`。

#### Scenario: npm 环境缺失

- **WHEN** 调用 `settings:agents:install`，agent 为 npx 类型，且系统中找不到 npm
- **THEN** 返回错误 `{ code: "ENV_MISSING", message: "需要先安装 Node.js" }`，不执行安装

#### Scenario: 安装成功

- **WHEN** 调用 `settings:agents:install`，agent 为 npx 类型，npm 可用
...
- **WHEN** 调用 `settings:agents:install`，agent 为 uvx 类型，且系统中找不到 uv
- **THEN** 返回错误 `{ code: "ENV_MISSING", message: "需要先安装 uv" }`，不执行安装

#### Scenario: 安装成功

- **WHEN** 调用 `settings:agents:install`，agent 为 uvx 类型，uv 可用
...
- **WHEN** 调用 `settings:agents:install`，agent 为 binary 类型，但 `distribution.binary` 中无当前平台的 entry
- **THEN** 返回错误 `{ code: "PLATFORM_UNSUPPORTED", message: "当前平台不支持此安装方式" }`

#### Scenario: 下载并安装成功

- **WHEN** 调用 `settings:agents:install`，agent 为 binary 类型，当前平台有对应 entry
...
- **WHEN** 已有 agent 正在安装时，调用 `settings:agents:install` 安装另一个 agent
