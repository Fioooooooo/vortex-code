...
agent spawn 命令根据 `AcpAgentEntry.distribution` 和 `AcpInstalledRecord` 组装，规则如下：

| 安装方式 | spawn 命令                                                                        | 参数                                                                           |
| -------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `npx`    | `npx`                                                                             | `["--no-install", distribution.npx.package, ...(distribution.npx.args ?? [])]` |
| `uvx`    | `uvx`                                                                             | `[distribution.uvx.package, ...(distribution.uvx.args ?? [])]`                 |
| `binary` | `join(getDataSubPath('acp'), 'bin', agent.id, distribution.binary[platform].cmd)` | `[]`（部分 agent 有 args，参考 registry）                                      |

`npx` 和 `uvx` 是全局命令，生产环境打包后无需特殊处理。`binary` 的可执行文件路径由 `installPath`（写入 `installed.json` 时记录）直接提供，无需重新计算。所有三种方式均使用 `stdio: ["pipe", "pipe", "inherit"]`，stdout/stdin 用于 ACP NDJSON 消息，stderr 透传日志。
...
### D3：ACP → SessionEvent 映射层，废弃 message_upsert/message_patch

**决定**：`acp-mapper.ts` 将 ACP `session/update` notification 转换为新的 `SessionEvent` 联合类型。ACP 的 tool call 是独立的一等公民事件，不依附于某条 assistant message，因此废弃旧的 `message_upsert`/`message_patch` 模式，改为直接映射 ACP tool call 语义。

| ACP sessionUpdate                 | SessionEvent                                                                                                                     |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `agent_message_chunk` (text)      | `text_delta`                                                                                                                     |
| `tool_call` (pending)             | `tool_call_start`（toolCallId, title, kind）                                                                                     |
| `tool_call_update` (any status)   | `tool_call_update`（toolCallId, status, content）                                                                                |
| `newSession`/`resumeSession` 返回 | `session_id_resolved`（acpSessionId，字段名从 `claudeSessionId` 改为 `acpSessionId`，触发时机从流中解析改为调用返回后立即 emit） |
| prompt 结束                       | `done`                                                                                                                           |
| 异常                              | `error`                                                                                                                          |

`MessageChunkData`（`shared/types/ipc.ts`）需同步更新，移除 `message_upsert`/`message_patch`，新增 `tool_call_start`/`tool_call_update`。

**前端 chat store 负责组装 assistant UIMessage**：ACP 没有"assistant message"的概念，text chunk 和 tool call 均为独立事件。store 维护 `activeAssistantId`，收到第一个 `text_delta` 或 `tool_call_start` 时创建 assistant message，后续事件追加到同一条消息的 `parts` 数组，`done` 时清空。主进程不参与消息组装，只做事件透传。

**理由**：旧的 `message_upsert`/`message_patch` 是为 Claude CLI 的"assistant message 包含 tool_use + 后续 user message 包含 tool_result"结构设计的，需要在 mapper 层缓存 pending tool call 等待 result 到达。ACP 的 tool call 是独立事件，有自己的 `toolCallId` 和状态流转（pending → in_progress → completed/failed），直接映射更自然，不需要缓存和合并逻辑。
...
**已解决**：`npx`/`uvx` 是全局命令，生产环境无需特殊处理。`binary` 的可执行路径在安装时已写入 `installed.json` 的 `installPath` 字段，直接读取即可。spawn 命令组装规则见 D1。
