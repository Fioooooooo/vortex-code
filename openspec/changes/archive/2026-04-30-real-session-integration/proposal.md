...
- **session 标题增加兜底策略**：若 ACP agent 未推送 `session_info_update`，新 session 的默认标题 SHALL 取首条用户消息去首尾空白、压缩连续空白后的前 30 个字符，避免列表长期停留在 `"New Session"`
...
- `session-management`：session 标题从固定 `"New Session"` 调整为"首条消息生成的兜底标题，可被后续 agent 推送覆盖"
...
- `frontend/src/stores/chat.ts`：新增首条用户消息兜底标题生成逻辑，并在草稿态创建 session 时传入该标题
...
- `frontend/src/__tests__/stores/chat.spec.ts`：补充兜底标题生成与后续标题覆盖的测试
- `openspec/changes/real-session-integration/tasks.md`：按新的草稿态 / 懒创建 / 标题兜底方案重新整理剩余实现任务
