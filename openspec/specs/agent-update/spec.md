...
对 `managedBy: "fyllocode"` 的 agent，SHALL 允许直接调用 `settings:agents:install` 执行更新（覆盖安装），无需额外确认。

#### Scenario: 一键更新

- **WHEN** 用户点击 `managedBy: "fyllocode"` 且 `updateAvailable: true` 的 agent 的"更新"按钮
- **THEN** 前端调用 `settings:agents:install`，流程与首次安装相同，完成后 `installed.json` 中 `installedVersion` 更新为新版本
...
- **THEN** 前端调用 `settings:agents:install`，安装完成后 `installed.json` 中 `managedBy` 更新为 `"fyllocode"`
